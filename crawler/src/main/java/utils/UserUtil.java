...
public class UserUtil {
	 ...
	 * 处理用户主页，析取用户信息
	 */
	public static void processUser(String userUrl) {
		User user = new User();
		...
		if (matcher.find()) {
			...
			user.setFollowingURL("https://www.zhihu.com/people/" + userID + "/following");
			user.setFollowerURL("https://www.zhihu.com/people/" + userID + "/followers");
		}
		// 解析页面中json格式的用户信息
		pattern = Pattern
		...
		if (jUser.getString("followingCount") != null) {
			user.setFollow(Long.parseLong(jUser.getString("followingCount")));
		}
		// 关注者
		if (jUser.getString("followerCount") != null) {
			user.setFollower(Long.parseLong(jUser.getString("followerCount")));
		}
		// 被赞数
		if (jUser.getString("voteupCount") != null) {
			user.setStarsNum(Long.parseLong(jUser.getString("voteupCount")));
		}
		// 个人简介
		if (jUser.getString("description") != null) {
			String description = jUser.getString("description");
			if (description.contains("&lt;br&gt;")) {
				description = description.replaceAll("&lt;br&gt;", ",");// 去掉<br>
			}
			if (description.contains("&lt;a")) {
				description = description.replaceAll("&lt;a\\shref.*?/a&gt;", "过滤链接");
			}
			user.setUserInfo(description);
		}
		// 被感谢数
		if (jUser.getString("thankedCount") != null) {
			user.setThxNum(Long.parseLong(jUser.getString("thankedCount")));
		}
		// 被收藏数
		if (jUser.getString("favoritedCount") != null) {
			user.setSaveNum((Long.parseLong(jUser.getString("favoritedCount"))));
		}
		// 匹配回答数量
		if (jUser.getString("answerCount") != null) {
			user.setAnswersNum(Long.parseLong(jUser.getString("answerCount")));
		}
		// 匹配行业
